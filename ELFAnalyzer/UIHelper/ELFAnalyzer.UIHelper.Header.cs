...
namespace PersonalTools.ELFAnalyzer.UIHelper
{
    public class ELFHeaderHelper
    {
        public static string GetFormattedELFHeaderInfo(ELFParser parser)
        {
            ...
            sb.AppendLine($"  类别:             {ELFHeaderInfo.GetELFClassName(parser.Header)} ({(parser.Is64Bit switch
            {
                true => "64-bit",
                _ => "32-bit"
            })})");
            sb.AppendLine($"  数据:             {ELFHeaderInfo.GetELFDataName(parser.Header)} ({(parser.Header.EI_DATA switch
            {
                (byte)ELFData.ELFDATA2LSB => "2's complement, little endian",
                _ => "2's complement, big endian"
            } )})");
            sb.AppendLine($"  版本:             {ELFHeaderInfo.GetReadableVersion(parser.Header)} ({parser.Header.EI_VERSION})");
            sb.AppendLine($"  OS/ABI:           {ELFHeaderInfo.GetOSABIName(parser.Header)} ({parser.Header.EI_OSABI})");
            sb.AppendLine($"  ABI 版本:         {parser.Header.EI_ABIVERSION}");
            sb.AppendLine($"  类型:             {ELFHeaderInfo.GetELFTypeName(parser.Header)} ({ELFHeaderInfo.GetFileTypeDescription(parser.Header)})");
            sb.AppendLine($"  系统架构:         {ELFHeaderInfo.GetArchitectureName(parser.Header)} ({ELFHeaderInfo.GetMachineDescription(parser.Header)})");
            sb.AppendLine($"  版本:             0x{parser.Header.e_version:X}");
            sb.AppendLine($"  入口点地址:       {ELFHeaderInfo.GetEntryPointAddress(parser.Header)}");
            sb.AppendLine($"  程序头起点:       {parser.Header.e_phoff} (bytes into file)");
            sb.AppendLine($"  节头的起点:       {parser.Header.e_shoff} (bytes into file)");
            sb.AppendLine($"  标志:             0x{parser.Header.e_flags:X}  {ELFHeaderInfo.GetFormattedELFFlags(parser.Header)}");
            sb.AppendLine($"  本头的大小:       {ELFHeaderInfo.GetHeaderSize(parser.Header)}");
