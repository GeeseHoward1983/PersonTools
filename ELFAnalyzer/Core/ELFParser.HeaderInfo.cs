...
namespace PersonalTools.ELFAnalyzer.Core
{
    internal static class HeaderInfo
    {
        ...
        // EI_OSABI 码 → 名称（数据表取代原 switch；未命中回退到 "OS/ABI Unknown"）
        private static readonly Dictionary<byte, string> s_osabiNames = new()
        {
            [0] = "UNIX - System V",
            [1] = "HP-UX",
            [2] = "NetBSD",
        ...
        private static readonly Dictionary<ushort, string> s_machineDescriptions = new()
        {
        ...
        {
            return s_machineDescriptions.TryGetValue(header.e_machine, out string? description)
                ? description
                : GetArchitectureName(header);
        }
        ...
        // (掩码, 标签) 表：按掩码顺序收集所有命中的标志位描述（纯位标志架构 PPC/SPARC/MIPS 共用）
        private static List<string> FormatFlagBits(uint flags, (uint Mask, string Label)[] table)
        {
            List<string> descriptions = [];
            foreach ((uint mask, string label) in table)
            {
                if ((flags & mask) != 0)
                {
