    ...
    internal static class PEResourceParserCertificate
    {
        ...
        internal static void ParseCertificateInfo(FileStream fs, BinaryReader reader, PEInfo peInfo)
        {
            try
            {
                ...
                List<string> certs = ReadCertificates(fs, reader, certificateOffset, certificateSize);
                if (certs.Count > 0)
                {
        ...
        // 解析安全目录中的 WIN_CERTIFICATE 数组（8字节对齐，支持多重签名），返回每个证书的描述
        private static List<string> ReadCertificates(FileStream fs, BinaryReader reader, uint certificateOffset, uint certificateSize)
        {
